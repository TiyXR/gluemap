...
from mast3r.catmlp_dpt_head_dg import mast3r_head_factory
...
class AsymmetricMASt3R(AsymmetricCroCo3DStereo):
    ...
    def set_dg_downstream_head(self, head_type_dg, output_mode_dg, **kw):
        self.output_mode_dg = output_mode_dg
        self.head_type_dg = head_type_dg
        self.head3 = mast3r_head_factory(
            head_type=head_type_dg, output_mode=output_mode_dg, net=self
        )
        self.head4 = mast3r_head_factory(
            head_type=head_type_dg, output_mode=output_mode_dg, net=self
        )

    def forward_ori(self, view1, view2):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if self.add_dg_pred_head:
                dopp_pred1 = self._downstream_head(
                    3, [tok.float() for tok in dec1], shape1
                )
                dopp_pred2 = self._downstream_head(
                    4, [tok.float() for tok in dec2], shape2
                )
    ...
    def forward(self, view1, view2, predict_extra=False):
        # encode the two images --> B,S,D
        (shape1, shape2), (feat1, feat2), (pos1, pos2) = self._encode_symmetrized(
            view1, view2
        )
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if predict_extra:
                # combine all ref images into object-centric representation
                dec3, dec4 = self._decoder(feat2, pos2, feat1, pos1)
                res3 = self._downstream_head(1, [tok.float() for tok in dec3], shape2)
                res4 = self._downstream_head(2, [tok.float() for tok in dec4], shape2)
        ...
        return_dense = (res1, res2, res3, res4) if predict_extra else (res1, res2)
        if self.add_dg_pred_head:
            return return_dense, dopp_pred1, dopp_pred2
        else:
            return return_dense
