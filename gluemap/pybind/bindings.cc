...
ceres::Manifold* CreatePoseManifold() {
  return new ceres::ProductManifold<ceres::EigenQuaternionManifold, ceres::EuclideanManifold<3>>();
}

// Helper function to create a ProductManifold for 7D pose with fixed translation component
ceres::Manifold* CreatePoseManifoldWithFixedTransComponent(int fixed_component) {
  std::vector<int> constant_indices = {fixed_component};
  return new ceres::ProductManifold<ceres::EigenQuaternionManifold, ceres::SubsetManifold>(
      ceres::EigenQuaternionManifold(),
      ceres::SubsetManifold(3, constant_indices)
  );
}

// Helper function to create a manifold that fixes rotation but allows translation
ceres::Manifold* CreateTranslationOnlyManifold() {
  ...
  return new ceres::ProductManifold<ceres::SubsetManifold, ceres::EuclideanManifold<3>>(
      ceres::SubsetManifold(4, constant_quat),
...
PYBIND11_MODULE(pygluemap, m)
{
  ...
  m.def(
      ...
      &colmap::CreateCameraCostFunction<ReprojErrorCostWithNegativeDepthFunctor, const Eigen::Vector2d &>,
      "camera_model_id"_a,
      "point2D"_a,
  ...
  m.def("CreatePoseManifoldWithFixedTransComponent",
        &CreatePoseManifoldWithFixedTransComponent,
        py::arg("fixed_component"),
        py::return_value_policy::take_ownership,
        "Create a ProductManifold for 7D pose with one translation component fixed.");

  m.def("CreateTranslationOnlyManifold",
        &CreateTranslationOnlyManifold,
        py::return_value_policy::take_ownership,
        "Create a manifold that fixes rotation but allows translation to vary.");

  // CUDA GPU solver
  m.def("solve_cuda", &SolveCUDA,
  ...
  // Numpy-based merge: returns a dict with merged 3D point arrays and updated dicts.
  // Python builds the pycolmap.Reconstruction from the returned data.
  m.def("compute_merge_data",
        &ComputeMergeDataWrapper,
        py::arg("tri_names"),
        py::arg("tri_ids"),
        py::arg("tri_n_pts2d"),
        py::arg("recon_names"),
        py::arg("recon_ids"),
        py::arg("recon_n_pts2d"),
        py::arg("recon_xyz"),
        py::arg("recon_track_img_ids"),
        py::arg("recon_track_pt2d_idxs"),
        py::arg("recon_track_lens"),
        py::arg("tri_xyz"),
        py::arg("tri_track_img_ids"),
        py::arg("tri_track_pt2d_idxs"),
        py::arg("tri_track_lens"),
        py::arg("virtual_point_start"),
        ...
        "tri_to_recon_id, tri_offsets, virtual_point_start, negative_depth_observations.");

  // Numpy-based track selection: returns point3D IDs to delete.
  // Python then calls reconstruction.delete_point3d(id) for each.
  m.def("compute_tracks_to_delete",
        &ComputeTracksToDeleteWrapper,
        py::arg("point3d_ids"),
        py::arg("track_image_ids"),
        py::arg("track_pt2d_idxs"),
        py::arg("track_lengths"),
        py::arg("virtual_point_start"),
        py::arg("sift_count"),
        py::arg("min_num_support_abs") = 512,
        py::arg("tracks_to_keep") = "sift",
        "Classify and select tracks. Returns int64 array of point3D IDs to delete.");
}
