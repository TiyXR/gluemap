...
MergeResult
MergeReconstructionData(const std::vector<std::string> &tri_names,
  ...
  for (const auto &[tri_id, recon_id] : tri_to_recon_id) {
    const int n_tri =
        tri_id_to_npts.count(tri_id) ? tri_id_to_npts.at(tri_id) : 0;
    const int n_recon =
        recon_id_to_npts.count(recon_id) ? recon_id_to_npts.at(recon_id) : 0;
  ...
  std::unordered_set<int64_t> merged_recon_ids(recon_ids.begin(),
                                               recon_ids.end());
  ...
  for (int64_t i = 0; i < N; ++i) {
    ...
    for (int64_t k = start; k < end; ++k) {
      ...
      if (!merged_recon_ids.count(img_id)) {
        valid = false;
        break;
      }
  ...
  for (int64_t i = 0; i < M; ++i) {
    ...
    for (int64_t k = start; k < end; ++k) {
      ...
      if (remap_it == tri_to_recon_id.end()) {
        valid = false;
        break;
      }
      const int64_t recon_img_id = remap_it->second;
      if (!merged_recon_ids.count(recon_img_id)) {
        valid = false;
        break;
      }
  ...
  std::cout << "Merged " << num_original << " original + " << num_merged
            << " triangulated 3D points, total " << result.merged_xyz.size()
            << std::endl;
  ...
  for (auto &[recon_id, neg_indices] : negative_depth_observations) {
    ...
    if (off_it != recon_offset.end() && off_it->second != 0) {
      ...
      for (const int idx : neg_indices)
        shifted.insert(idx + offset);
...
py::dict ComputeMergeDataWrapper(
  ...
  std::vector<int64_t> tri_ids_vec(tri_ids.data(),
                                   tri_ids.data() + tri_ids.size());
  std::vector<int32_t> tri_n_pts2d_vec(tri_n_pts2d.data(),
                                       tri_n_pts2d.data() + tri_n_pts2d.size());
  std::vector<int64_t> recon_ids_vec(recon_ids.data(),
                                     recon_ids.data() + recon_ids.size());
  std::vector<int32_t> recon_n_pts2d_vec(
      recon_n_pts2d.data(), recon_n_pts2d.data() + recon_n_pts2d.size());
  std::vector<int64_t> recon_img_ids_vec(recon_track_img_ids.data(),
                                         recon_track_img_ids.data() +
                                             recon_track_img_ids.size());
  std::vector<int32_t> recon_pt2d_vec(recon_track_pt2d_idxs.data(),
                                      recon_track_pt2d_idxs.data() +
                                          recon_track_pt2d_idxs.size());
  std::vector<int32_t> recon_lens_vec(recon_track_lens.data(),
                                      recon_track_lens.data() +
                                          recon_track_lens.size());
  std::vector<int64_t> tri_img_ids_vec(tri_track_img_ids.data(),
                                       tri_track_img_ids.data() +
                                           tri_track_img_ids.size());
  std::vector<int32_t> tri_pt2d_vec(tri_track_pt2d_idxs.data(),
                                    tri_track_pt2d_idxs.data() +
                                        tri_track_pt2d_idxs.size());
  std::vector<int32_t> tri_lens_vec(
      tri_track_lens.data(), tri_track_lens.data() + tri_track_lens.size());
  ...
  for (int64_t i = 0; i < recon_xyz.shape(0); ++i)
    recon_xyz_vec[i] = {recon_xyz_r(i, 0), recon_xyz_r(i, 1),
                        recon_xyz_r(i, 2)};
  ...
  MergeResult r = MergeReconstructionData(
      tri_names, tri_ids_vec, tri_n_pts2d_vec, recon_names, recon_ids_vec,
      recon_n_pts2d_vec, recon_xyz_vec, recon_img_ids_vec, recon_pt2d_vec,
      recon_lens_vec, tri_xyz_vec, tri_img_ids_vec, tri_pt2d_vec, tri_lens_vec,
  ...
  result["merged_track_img_ids"] =
      VecToArray1D(std::move(r.merged_track_img_ids));
  result["merged_track_pt2d_idxs"] =
      VecToArray1D(std::move(r.merged_track_pt2d_idxs));
