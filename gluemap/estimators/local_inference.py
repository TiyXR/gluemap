...
def create_local_inference(model, model_type, device="cuda", dtype=torch.bfloat16):
    ...
    if model_type in ("pi3", "pi3x"):
        from gluemap.estimators.pi3_inference import Pi3LocalInference

        return Pi3LocalInference(model, device, dtype)
    elif model_type == "vggt":
        from gluemap.estimators.vggt_inference import VGGTLocalInference

        return VGGTLocalInference(model, device, dtype)
    elif model_type == "map_anything":
        from gluemap.estimators.mapanything_inference import MapAnythingLocalInference
